...
TCameraInfo = collections.namedtuple("TCameraInfo",
                                     ["cam_id", "dev_id", "sens_id", "model", "serial_number", "in_use", "status"])
...
def list_cameras(backend="uc480"):
    ...
    """
    lib = get_lib(backend)
    return [TCameraInfo(ci.dwCameraID, ci.dwDeviceID, ci.dwSensorID, py3.as_str(ci.Model), py3.as_str(ci.SerNo),
                        bool(ci.dwInUse), ci.dwStatus)
...
TDeviceInfo = collections.namedtuple("TDeviceInfo",
                                     ["cam_id", "model", "manufacturer", "serial_number", "usb_version", "date",
                                      "dll_version", "camera_type"])
TAcquiredFramesStatus = collections.namedtuple("TAcquiredFramesStatus",
                                               ["acquired", "transfer_missed", "frameskip_events"])
TTimestamp = collections.namedtuple("TTimestamp", ["year", "month", "day", "hour", "minute", "second", "millisecond"])
TFrameInfo = collections.namedtuple("TFrameInfo",
                                    ["frame_index", "framestamp", "timestamp", "timestamp_dev", "size", "io_status",
                                     "flags"])
...
class UC480Camera(camera.IBinROICamera, camera.IExposureCamera):
    ...
    _frameinfo_fields = general.make_flat_namedtuple(TFrameInfo, fields={"timestamp": TTimestamp,
                                                                         "size": camera.TFrameSize})._fields
    ...
    def open(self):
        ...
        if self.hcam is None:
            self.hcam = self.lib.is_InitCamera(self.id | (uc480_defs.DEVENUM.IS_USE_DEVICE_ID if self.is_dev_id else 0),
                                               None)
    ...
    def get_device_info(self):
        ...
        return TDeviceInfo(cam_id, py3.as_str(sen_info.strSensorName), py3.as_str(cam_info.ID),
                           py3.as_str(cam_info.SerNo), py3.as_str(cam_info.Version),
    ...
    _p_trigger_mode = interface.EnumParameterClass("trigger_mode",
                                                   {"int": uc480_defs.TRIGGER.IS_SET_TRIGGER_OFF,
                                                    "software": uc480_defs.TRIGGER.IS_SET_TRIGGER_SOFTWARE,
                                                    "ext_rise": uc480_defs.TRIGGER.IS_SET_TRIGGER_LO_HI,
                                                    "ext_fall": uc480_defs.TRIGGER.IS_SET_TRIGGER_HI_LO,
                                                    "ext": uc480_defs.TRIGGER.IS_SET_TRIGGER_LO_HI})
    ...
    def _allocate_buffers(self, n):
        ...
        self._buffers = []
        for _ in range(n):
            self._buffers.append((self.lib.is_AllocImageMem(self.hcam, frame_size[1], frame_size[0], bpp),
                                  (frame_size[0], frame_size[1]), bpp))
            # self._buffers.append((self.lib.is_AllocImageMem(self.hcam, 2048,2048,bpp),(2048,2048),bpp))
    ...
    def _update_buffer_counter(self, timeout=None, skip_gap=False):
        ...
        while True:
            ...
            if ctd.passed():
                return False
        last_acq = self.lib.is_CameraStatus(self.hcam, uc480_defs.CAMINFO.IS_SEQUENCE_CNT,
                                            uc480_defs.CAMINFO.IS_GET_STATUS) + self._acq_offset - 1
        last_buffer = bs[2]
    ...
    def get_frame_timings(self):
        exp = self.lib.is_Exposure(self.hcam, uc480_defs.EXPOSURE_CMD.IS_EXPOSURE_CMD_GET_EXPOSURE,
    ...
    def set_exposure(self, exposure):
        ...
        exposure = self.lib.is_Exposure(self.hcam, uc480_defs.EXPOSURE_CMD.IS_EXPOSURE_CMD_SET_EXPOSURE,
                                        ctypes.c_double, exposure * 1E3)
    ...
    def get_available_pixel_rates(self):
        ...
        nrates = self.lib.is_PixelClock(self.hcam, uc480_defs.PIXELCLOCK_CMD.IS_PIXELCLOCK_CMD_GET_NUMBER,
                                        ctypes.c_uint)
        rates = self.lib.is_PixelClock(self.hcam, uc480_defs.PIXELCLOCK_CMD.IS_PIXELCLOCK_CMD_GET_LIST,
                                       ctypes.c_uint * nrates)
    ...
    def get_pixel_rates_range(self):
        ...
        """
        rng = self.lib.is_PixelClock(self.hcam, uc480_defs.PIXELCLOCK_CMD.IS_PIXELCLOCK_CMD_GET_RANGE,
                                     ctypes.c_uint * 3)
    ...
    def set_pixel_rate(self, rate=None):
        ...
        self.lib.is_PixelClock(self.hcam, uc480_defs.PIXELCLOCK_CMD.IS_PIXELCLOCK_CMD_SET, ctypes.c_uint,
                               int(np.round(rate / 1E6)))
    ...
    _color_modes = {"raw8": uc480_defs.COLORMODE.IS_CM_SENSOR_RAW8, "raw10": uc480_defs.COLORMODE.IS_CM_SENSOR_RAW10,
                    ...
                    "bgr5p": uc480_defs.COLORMODE.IS_CM_BGR5_PACKED,
                    "bgr565p": uc480_defs.COLORMODE.IS_CM_BGR565_PACKED,
                    "rgb8p": uc480_defs.COLORMODE.IS_CM_RGB8_PACKED, "bgr8p": uc480_defs.COLORMODE.IS_CM_BGR8_PACKED,
                    "rgba8p": uc480_defs.COLORMODE.IS_CM_RGBA8_PACKED,
                    "bgra8p": uc480_defs.COLORMODE.IS_CM_BGRA8_PACKED,
                    "rgby8p": uc480_defs.COLORMODE.IS_CM_RGBY8_PACKED,
                    "bgry8p": uc480_defs.COLORMODE.IS_CM_BGRY8_PACKED,
                    "rgb10p": uc480_defs.COLORMODE.IS_CM_RGB10_PACKED,
                    "bgr10p": uc480_defs.COLORMODE.IS_CM_BGR10_PACKED,
                    "rgb10up": uc480_defs.COLORMODE.IS_CM_RGB10_UNPACKED,
                    "bgr10up": uc480_defs.COLORMODE.IS_CM_BGR10_UNPACKED,
                    "rgb12up": uc480_defs.COLORMODE.IS_CM_RGB12_UNPACKED,
                    "bgr12up": uc480_defs.COLORMODE.IS_CM_BGR12_UNPACKED,
                    "rgba12up": uc480_defs.COLORMODE.IS_CM_RGBA12_UNPACKED,
                    "bgra12up": uc480_defs.COLORMODE.IS_CM_BGRA12_UNPACKED,
                    "cbycryp": uc480_defs.COLORMODE.IS_CM_CBYCRY_PACKED,
                    "uyuvp": uc480_defs.COLORMODE.IS_CM_UYVY_PACKED,
                    "uyvy_monop": uc480_defs.COLORMODE.IS_CM_UYVY_MONO_PACKED,
                    "uyuv_bayerp": uc480_defs.COLORMODE.IS_CM_UYVY_BAYER_PACKED,
    ...
    _mode_properties = {"raw8": (8, 1), "raw10": (16, 1), "raw12": (16, 1), "raw16": (16, 1),
                        # needs additional decoding
    ...
    def get_gains(self):
        ...
        """
        return tuple(
            [self.lib.is_SetHWGainFactor(self.hcam, uc480_defs.GAINFACTOR.IS_GET_MASTER_GAIN_FACTOR + i, 0) / 100 for i
             in range(4)])
    ...
    def get_max_gains(self):
        ...
        """
        return tuple(
            [self.lib.is_SetHWGainFactor(self.hcam, uc480_defs.GAINFACTOR.IS_INQUIRE_MASTER_GAIN_FACTOR + i, 100) / 100
             for i in range(4)])
    ...
    def get_gain_boost(self):
        ...
        return bool(self.lib.is_SetGainBoost(self.hcam,
                                             uc480_defs.GAIN.IS_GET_SUPPORTED_GAINBOOST) and self.lib.is_SetGainBoost(
            self.hcam, uc480_defs.GAIN.IS_GET_GAINBOOST))
    ...
    def _get_acquired_frames(self, error_on_skip=True):  # pylint: disable=arguments-differ
        acq = self.lib.is_CameraStatus(self.hcam, uc480_defs.CAMINFO.IS_SEQUENCE_CNT,
                                       uc480_defs.CAMINFO.IS_GET_STATUS) + self._acq_offset
        ...
        if acq < prev_acq:
            ...
            if error_on_skip:
                raise self.FrameTransferError(
                    "acquisition restart detect: last acquired frame is {}, next acquired frame is {}".format(prev_acq,
                                                                                                              acq))
            return prev_acq
        return acq
    ...
    _subsampling_modes = {("v", 1): 0, ("h", 1): 0,
                          ("v", 2): uc480_defs.SUBSAMPLING.IS_SUBSAMPLING_2X_VERTICAL,
                          ("h", 2): uc480_defs.SUBSAMPLING.IS_SUBSAMPLING_2X_HORIZONTAL,
                          ("v", 3): uc480_defs.SUBSAMPLING.IS_SUBSAMPLING_3X_VERTICAL,
                          ("h", 3): uc480_defs.SUBSAMPLING.IS_SUBSAMPLING_3X_HORIZONTAL,
                          ("v", 4): uc480_defs.SUBSAMPLING.IS_SUBSAMPLING_4X_VERTICAL,
                          ("h", 4): uc480_defs.SUBSAMPLING.IS_SUBSAMPLING_4X_HORIZONTAL,
                          ("v", 5): uc480_defs.SUBSAMPLING.IS_SUBSAMPLING_5X_VERTICAL,
                          ("h", 5): uc480_defs.SUBSAMPLING.IS_SUBSAMPLING_5X_HORIZONTAL,
                          ("v", 6): uc480_defs.SUBSAMPLING.IS_SUBSAMPLING_6X_VERTICAL,
                          ("h", 6): uc480_defs.SUBSAMPLING.IS_SUBSAMPLING_6X_HORIZONTAL,
                          ("v", 8): uc480_defs.SUBSAMPLING.IS_SUBSAMPLING_8X_VERTICAL,
                          ("h", 8): uc480_defs.SUBSAMPLING.IS_SUBSAMPLING_8X_HORIZONTAL,
                          ("v", 16): uc480_defs.SUBSAMPLING.IS_SUBSAMPLING_16X_VERTICAL,
                          ("h", 16): uc480_defs.SUBSAMPLING.IS_SUBSAMPLING_16X_HORIZONTAL}
    ...
    _binning_modes = {("v", 1): 0, ("h", 1): 0,
                      ("v", 2): uc480_defs.BINNING.IS_BINNING_2X_VERTICAL,
                      ("h", 2): uc480_defs.BINNING.IS_BINNING_2X_HORIZONTAL,
                      ("v", 3): uc480_defs.BINNING.IS_BINNING_3X_VERTICAL,
                      ("h", 3): uc480_defs.BINNING.IS_BINNING_3X_HORIZONTAL,
                      ("v", 4): uc480_defs.BINNING.IS_BINNING_4X_VERTICAL,
                      ("h", 4): uc480_defs.BINNING.IS_BINNING_4X_HORIZONTAL,
                      ("v", 5): uc480_defs.BINNING.IS_BINNING_5X_VERTICAL,
                      ("h", 5): uc480_defs.BINNING.IS_BINNING_5X_HORIZONTAL,
                      ("v", 6): uc480_defs.BINNING.IS_BINNING_6X_VERTICAL,
                      ("h", 6): uc480_defs.BINNING.IS_BINNING_6X_HORIZONTAL,
                      ("v", 8): uc480_defs.BINNING.IS_BINNING_8X_VERTICAL,
                      ("h", 8): uc480_defs.BINNING.IS_BINNING_8X_HORIZONTAL,
                      ("v", 16): uc480_defs.BINNING.IS_BINNING_16X_VERTICAL,
                      ("h", 16): uc480_defs.BINNING.IS_BINNING_16X_HORIZONTAL}
    ...
    def get_roi(self):
        ...
        return (
        rect.s32X * hbin, (rect.s32X + rect.s32Width) * hbin, rect.s32Y * vbin, (rect.s32Y + rect.s32Height) * vbin,
        hbin, vbin)
    ...
    def _frame_info_to_namedtuple(self, info):
        return self._TFrameInfo(info[0], info[1], TTimestamp(*info[2:9]), info[9], camera.TFrameSize(*info[10:12]),
                                *info[12:14])
    ...
    def _read_buffer(self, n, return_info=False, nchan=None):
        ...
        if return_info:
            ...
            frame_info = TFrameInfo(n, frame_info.u64FrameNumber, ts, frame_info.u64TimestampDevice, size,
                                    frame_info.dwIoStatus, frame_info.dwFlags)
        return frame, frame_info

    def _read_frames(self, rng, return_info=False):
        nchan = self._get_pixel_mode_settings()[1]
        data = [self._read_buffer(n, return_info=return_info and (n % self._frameinfo_period == 0), nchan=nchan) for n
                in range(rng[0], rng[1])]
